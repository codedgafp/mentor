...
class block_summary extends block_base {
    ...
    function init() {
        ...
        if($COURSE->format == 'modular'){
            ...
            && has_capability('block/summary:canseesectionzero', context_course::instance($COURSE->id))){
    ...
    function get_content() {
        ...
        if($COURSE->format == 'modular'){
            ...
            if($this->modularparentsection){
                ...
                $subheader = html_writer::link($parenturl, (section_is_completed($this->modularparentsection->id)?'<i class="fa fa-check"></i> ':'').$this->modularparentsection->name);
    ...
    private function generate_content_for_other_format($context){
        ...
        {
            // Patch Edunao Mentor : postgres compatibility.
            $section = $DB->get_record_sql('SELECT * FROM {course_sections} WHERE course = :courseid AND sequence LIKE \'%' . $this->page->cm->id . '%\'', ['courseid' => $COURSE->id]);
        ...
        foreach($tree as $page){
            $sum_structure_diplay .= $this->display_page_structure($page,$currentsectionid);
        }
        
        // Patch Edunao Mentor : ajout du role et du aria-label.
	$this->content->text = html_writer::tag('ul', $sum_structure_diplay, ['role' => 'navigation', 'aria-label' => 'Sommaire']);
    ...
    private function display_page_structure($node, $currentsection){
    ...
    private function get_last_weight(){
    	...
    	$sql = "SELECT * FROM(
				SELECT bi.defaultweight as weight FROM {block_instances} bi WHERE bi.parentcontextid = $contextid AND bi.defaultregion = 'side-pre'
				UNION
				SELECT bp.weight FROM {block_positions} bp WHERE bp.contextid = $contextid AND bp.region = 'side-pre') res
				ORDER by res.weight";
